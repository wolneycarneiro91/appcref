...
{
	var a = await resultadoAnamnese();
	var p = await resultadoPARQ();
	var r = await resultadoRisco();
    if (a <= 1)
    {
       a = 0;
    }
	...
	else if((a == 1 && r <= 2) || (p >= 1 ))
	{
		...
		$("#pgn_avaliacao_conduta").html("<h5>Conduta do profissional de Educação Física</h5> Início do programa de atividade física de baixa intensidade e encaminhamento para avaliação médica conforme necessidade observada na avaliação. O profissional de Educação Física deverá estabelecer um prazo para que o atestado médico, bem como os resultados dos exames sejam apresentados.");
